"""
pages/2_classificator.py
────────────────────────
Страница LLM-классификатора. Использует единое хранилище данных из session_store.
"""
...
from src.session_store import (
    ...
    is_data_loaded, get_source_name,
...
if st.button("▶️ Запустить классификацию", key="classifier_run", type="primary"):
    try:
        st.info("⏳ Этап 0: Загрузка данных...")
        
        # Берём данные из session_store (сырой xlsx)
        ...
        if os.path.exists(LLM_FINAL_OUTPUT_FILE):
            ...
            # ─── Сохраняем результат в session_store для других страниц ─────
            result_pl = pl.from_pandas(result_df)
            set_processed_data(result_pl, get_source_name(), "AI-решение (slow)")
            st.success("✅ Результат классификации сохранён в хранилище — теперь доступен на страницах Карта и Аналитик")
