...
from src.map_utils.session_store import (
    get_processed_data,
    is_data_loaded,
    get_source_name,
...
# ─── Загрузка данных через единое хранилище ──────────────────────────────────
...
if is_data_loaded():
    ...
    if df is None:
        df = df_loaded  # используем данные из render_sidebar_upload()

    if df is not None:
        st.success(f"✅ Данные загружены: {get_source_name()} | Записей: {len(df)}")
    ...
    st.info("⬅️ Загрузите .xlsx файл через боковую панель, и данные станут доступны здесь.")
